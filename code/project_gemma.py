...
def format_prompt(prompt_type, story0, story1, model_name, examples=None, knowledge=None):
    if prompt_type == "zero-shot":
        ...
        for eg in examples:
            ...
            prompt += f"Story A: {storyA}\nStory B: {storyB}\n\n #### {label}\n"
            for pair_list in confl_pairs:
                prompt += f"%%%% {pair_list[0]}, {pair_list[1]}\n"
        ...
        know_s0 = knowledge[0].split("<start_of_turn>model")[1]
        know_s1 = knowledge[1].split("<start_of_turn>model")[1]
...
def zero_shot_prompting(model, test_data, model_name):
    ...
    for example in test_data:
        ...
        print("TRUNCATED_RESPONSE:", response)
...
def generate_knowledge(model, story0, story1):
    KNOWLEDGE_PROMPT_S0 = f"""<start_of_turn>user
    ...
    <start_of_turn>model
    """ 
    KNOWLEDGE_PROMPT_S1 = f"""<start_of_turn>user
    Generate knowledge on the impact of an action on the physical state of objects and the plausibility of the action. Examples:
    Input: 
    1. John played on the swings.\n
    2. John went towards the bench.\n
    ...
    <start_of_turn>model
    """ 
    s0_know = model(KNOWLEDGE_PROMPT_S0, max_length=4096, num_return_sequences=1)[0]['generated_text']
    s1_know = model(KNOWLEDGE_PROMPT_S1, max_length=4096, num_return_sequences=1)[0]['generated_text']
...
def main():
    ...
    if method == 'zero-shot':
        ...
        acc_predictions, con_predictions = cot_icl_prompting(model, dataset, model_name, examples)
    else:  # gen-know
